function AS () {
	var
		dbFile = Components.classes["@mozilla.org/file/directory_service;1"].getService(Components.interfaces.nsIProperties).get("ProfD", Components.interfaces.nsIFile),
	...
	this.init = function(){
		this.dbTablesCheckAndCreate();
		myPrefObserver.register();//?
		this.setSidebarWidth();
		}

	this.dbTablesCheckAndCreate = function(){
		...
		if(!this.db.tableExists("searches")){
			...
			for (var i = 997; i < 1000; i++) {
				...
				//997 - поля по умолчанию, 998 - всё по нулям, 999 - autosave
				}
			}
		}
